...
namespace SourceLiveTimer.Demo
{
	internal class Portal2CoopCourse6GameHandler : OrangeBoxGameHandler
	{
        ...
        private string[] _maps = Category.PORTAL_2_COOP_COURSE_6.Maps;
		...
		protected override PacketResult ProcessPacket(BinaryReader br)
		{
			...
			stringBuilder.Append(string.Concat(currentTick));
			if (this._startAdjustType == null && packetResult.CurrentPosition.Equals(new Vector(-9896f, -4400f, 3048f)))
			{
				this._startAdjustType = "Portal2 co-op Atlas Gain Control";
				this._startTick = base.CurrentTick;
			}
			else if (packetResult.CurrentPosition.Equals(new Vector(-11168f, -4384f, 3040.03125f)))
			{
