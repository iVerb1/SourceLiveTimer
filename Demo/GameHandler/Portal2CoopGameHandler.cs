...
namespace SourceLiveTimer.Demo
{
	internal class Portal2CoopGameHandler : OrangeBoxGameHandler
	{
        ...
        private string[] _maps = Category.PORTAL_2_COOP.Maps;
		...
		protected override PacketResult ProcessPacket(BinaryReader br)
		{
			...
			stringBuilder.Append(string.Concat(currentTick));
			if (this._startAdjustType == null && base.Map == "mp_coop_start" && packetResult.CurrentPosition.Equals(new Vector(-9896f, -4400f, 3048f)))
			{
				this._startAdjustType = "Portal2 co-op Atlas Gain Control";
				this._startTick = base.CurrentTick;
			}
			else if (base.Map == "mp_coop_start" && packetResult.CurrentPosition.Equals(new Vector(-11168f, -4384f, 3040.03125f)))
			{
